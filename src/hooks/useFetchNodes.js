...
export const useFetchNodes = ({
  ...
  const mapDataCache = useSelector((state) => state.mapData.cache);
  ...
  const fetchNodes = async (language_id = null) => {
    try {
      ...
      // Generate Cache Key
      const cacheKey = `${Process_id}_${levelParam}_${language_id || 'def'}_${mode}`;

      // Check Cache
      if (mapDataCache[cacheKey]) {
        console.log("Serving from Cache:", cacheKey);
        const cachedData = mapDataCache[cacheKey];
        // Apply cached data (same logic as below but with cached object)
      ...
      // Save to Cache
      // Note: We're caching the raw API response *before* node processing, 
      // or we can cache the processed nodes. 
      // Caching the API response allows us to re-run node processing logic if needed (e.g. label change handlers).
      // However, node processing involves async checks. 

      // Let's proceed with processing, then cache the result? 
      // Actually, standard Redux pattern is to store serializable data. 
      // The API response is serializable.

      if (data) {
        dispatch(setMapData({ key: cacheKey, data }));
  ...
  // Close applyData function and return from hook
  return { fetchNodes };
};
