          ...
          setChiledNodes((prevNodes) =>
            prevNodes.filter((node) => {
              ...
              return !removedEdges.some((edge) =>
                ...
                (edge.source === node.data.sourceNodeId && edge.target === node.data.targetNodeId &&
                  edge.sourceHandle === node.data.sourceHandle && edge.targetHandle === node.data.targetHandle)
              );
            })
          );
        }
    ...
    setChiledNodes((prevNodes) =>
      prevNodes.filter((node) => {
        ...
        return !(
          ...
          (node.data.sourceNodeId === edgeToDelete.source &&
            node.data.targetNodeId === edgeToDelete.target &&
            node.data.sourceHandle === edgeToDelete.sourceHandle &&
            node.data.targetHandle === edgeToDelete.targetHandle)
        );
      })
    );
    setSelectedEdge(null);
